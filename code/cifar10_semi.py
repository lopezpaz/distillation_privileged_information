...
def downsample(x,p_down):
    size = len(imresize(x[0].reshape(3,32,32),p_down,mode='RGB').ravel())
    s_tr = np.zeros((x.shape[0], size))
    for i in xrange(x.shape[0]):
      img = x[i].reshape(3,32,32)
      s_tr[i] = imresize(img,p_down,mode='RGB').ravel()
    return s_tr
...
ax_tr, ay_tr, x_te, y_te = load_data('cifar10')
p_downsample = 15
N = 300 
M = 20

outfile = open('result_cifar10_semi3', 'w')

x_te  = x_te/255.0
xs_te = x_te + np.random.randn(x_te.shape[0], x_te.shape[1])

for rep in xrange(4):
  # random training split
  lab  = np.random.permutation(ax_tr.shape[0])[0:N]
  ulab = np.setdiff1d(xrange(ax_tr.shape[0]), lab)
  y_tr = np.copy(ay_tr)
  y_tr[ulab] *= 0
  x_tr  = ax_tr/255.0
  xs_tr = x_tr+np.random.randn(x_tr.shape[0], x_tr.shape[1])

  # big mlp
  mlp_big = MLP(x_tr.shape[1],M,y_tr.shape[1])
  mlp_big.fit(x_tr[lab], y_tr[lab], nb_epoch=50, verbose=0)
